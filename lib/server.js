      ...
      switch (RequestCase) {
        ...
        case 2:
          ...
          for (var i = 0; i < e1[0].RId.length; i++) {
            var eMid = responses[e1[0].RId[i]].entities;
            eMid[0].AA.find(function(ele){
              if (eMid[0].AA) {
                for (var j = 0; j < eMid[0].AA.length; j++) {
                  if (id2 in eMid[0].AA[j].AuId) {
                  twoHop.push([id1, id2]);
                  break;
                  }
                }
        ...
        case 3:
          ...
          for (var i = 0; i < e2[0].RId.length; i++) {
            var eMid = responses[e2[0].RId[i]].entities;
            eMid[0].AA.find(function(ele){
              if (eMid[0].AA) {
                for (var j = 0; j < eMid[0].AA.length; j++) {
                  if (id2 in eMid[0].AA[j].AuId) {
        ...
        case 4:
          ...
          for (var i = 0; i < e1.length; i++) {
            e1[i].AA.find(function(ele) {
              if (e1[i].AA) {
                for (var j = 0; j < e1[i].AA.length; j++) {
                  if (id2 in e1[i].AA[j].AuId) {
                  twoHop.push([id1, id2]);
                  break;
                }
              }              
          ...
          if (responses[Id1].AfId && responses[Id2].AfId) {
            if (responses[Id1].AfId == responses[Id2].AfId) {
              twoHop.push([id1, id2]);
            }
          }          
          ...
          for (var RId1Idx = 0; RId1Idx < RId1Arr.length; RId1Idx++) {
            ...
            for (var RId2Idx = 0; RId2Idx < RId2Arr.length; RId2Idx++) {
              ThirdLevelPromiseArr.push(new Promise(function(resolve, reject) {
                ...
                // cache hit~
                if (responses[currRId.toString()]) { console.log('hit in 4id');resolve(); }
                client.getId(currRId, function(err, obj) {
                  if (!err) {
                    // cache it
                    responses[currRId.toString()] = obj;
